...
interface JobsApiResponse {
  jobs: JobListing[];
  directHireOpportunities: DirectHireOpportunity[];
  total: number;
  offset: number;
  limit: number;
  facets: JobFacets;
...
export function HomepageSearch({ initialData, initialFilters }: HomepageSearchProps) {
  ...
  const [directHireOpportunities, setDirectHireOpportunities] = useState<DirectHireOpportunity[]>(
    initialData?.directHireOpportunities ?? [],
  );
  ...
  const runSearch = useCallback(
    async (append: boolean, offsetOverride = 0) => {
      ...
      try {
        ...
        setDirectHireOpportunities(data.directHireOpportunities ?? []);
        ...
        >
          <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-5">
              <div>
                <p className="eyebrow mb-2">Aktuelle Strömung</p>
                <h2 className="text-2xl sm:text-3xl font-black text-foreground">
                  {hasSearched ? "Passende Klimastellen" : "Aktuelle Klimastellen"}
                </h2>
                {hasActiveLocation && (
              ...
              {(totalJobs > 0 || directHireOpportunities.length > 0) && (
                <span className="font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  ...
                  konkrete {totalJobs === 1 ? "Stelle" : "Stellen"}
                  {directHireOpportunities.length > 0 && (
                    <> · {directHireOpportunities.length} Direktanstellungsprofile</>
                  )}
                </span>
              )}
            </div>
            ...
            {!isLoading && !errorMessage && directHireOpportunities.length > 0 && (
              <div
                className="source-panel mb-4 px-4 py-3 text-sm text-foreground"
                role="status"
                aria-live="polite"
                data-nosnippet=""
              >
                <p className="font-semibold">
                  {visibleJobs} {visibleJobs === 1 ? "konkrete Stelle passt" : "konkrete Stellen passen"} zur aktuellen Auswahl.
                </p>
                <p className="mt-1">
                  Ergänzend zeigen wir {directHireOpportunities.length} kontrollierte Direktanstellungsprofile. Sie sind keine konkreten offenen Stellen und enthalten weder Arbeitgeber- noch Lohnversprechen.
                </p>
              </div>
            )}
            ...
            {!isLoading && !errorMessage && directHireOpportunities.length > 0 && (
              <div className={`space-y-3 sm:space-y-4 ${jobs.length > 0 ? "mt-4" : ""}`}>
                {directHireOpportunities.map((opportunity) => (
                  <article
                    key={opportunity.id}
                    aria-labelledby={`direct-hire-title-${opportunity.id}`}
                    data-nosnippet=""
                    className="min-w-0"
                  >
                    <Card className="job-card direct-hire-card gap-0 py-0">
                      <CardContent className="p-5 pl-6 sm:p-6 sm:pl-7">
                        <div className="mb-3 flex min-w-0 flex-wrap items-center gap-2">
                          ...
                          <h3
                            id={`direct-hire-title-${opportunity.id}`}
                        ...
                        <div className="job-facts">
                          ...
                          <div className="job-fact sm:col-span-3">
                            <span className="job-fact__value [overflow-wrap:anywhere]">
                              {opportunity.preferenceSummary}
                            </span>
                            <span className="job-fact__label">Deine Präferenzen</span>
                          </div>
                        </div>
                        ...
                        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 text-xs font-semibold text-slate-600">
                              Kein Arbeitgeber, Lohn oder Stellenantritt ist zugesichert.
                            </p>
                          </div>
                          <Link
