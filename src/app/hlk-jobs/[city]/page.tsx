...
export async function generateStaticParams() {
  return HLK_CITIES.map((city) => ({ city: city.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findHlkCity(citySlug);
  if (!city) return { title: "Kältesystem-Monteur Jobs" };

  const title = `Kältesystem-Monteur Jobs ${city.name}`;
  const description = `Stelleninserate mit Klimabezug für ${city.name} und Umgebung. Arbeitsort und Anforderungen im jeweiligen Inserat prüfen.`;
  ...
  return {
    ...
    // No real inventory/editorial gate exists for these programmatic city pages.
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Startseite", item: SITE_URL },
      {
        "@type": "ListItem",
        position: 2,
        name: `Kältesystem-Monteur Jobs ${city.name}`,
      ...
      <main id="main-content" className="bg-background">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            <nav className="text-sm text-slate-500 mb-3" aria-label="Breadcrumb">
              ...
              <span className="text-slate-700">Kältesystem-Monteur Jobs {city.name}</span>
            </nav>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight mb-4">
              Kältesystem-Monteur Jobs <span className="text-primary">{city.name}</span>
            </h1>
            <p className="text-slate-600 text-lg leading-relaxed mb-4 max-w-3xl">
              Diese Seite zeigt Suchtreffer mit Klimabezug für {city.name}.
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Klima-Berufsbezeichnungen in der Suche
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 pb-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            {HLK_CITIES.filter((item) => item.slug !== city.slug).map((item) => (
              <Link
                key={item.slug}
                href={`/hlk-jobs/${item.slug}`}
              ...
              >
                Kältesystem-Monteur Jobs {item.name}
              </Link>
            ))}
          </div>
        </section>
