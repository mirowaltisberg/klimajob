...
import { HLK_CITIES, findHLKCity } from "@/lib/hlk-cities";
...
const ROLE_LINKS: { slug: string; label: string }[] = [
  { slug: "kaeltetechniker-efz", label: "Kältetechniker EFZ" },
  { slug: "kaeltemonteur-efz", label: "Kältemonteur EFZ" },
  { slug: "lueftungsmonteur-efz", label: "Lüftungsmonteur EFZ" },
  { slug: "klimatechniker", label: "Klimatechniker" },
  { slug: "servicetechniker-hlk", label: "Servicetechniker HLK" },
  { slug: "projektleiter-hlk", label: "Projektleiter HLK" },
];

export async function generateStaticParams() {
  return HLK_CITIES.map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findHLKCity(citySlug);
  if (!city) return { title: "Kältetechniker Jobs" };

  const title = `Kältetechniker Jobs ${city.name} 2026 | Stellen für Kältetechniker in ${city.name}`;
  const description = `Offene Kältetechniker Stellen in ${city.name} (${city.cantonAbbr}). Lohn ${city.salaryBand}, tägliche Updates aus der ${city.region}.`;

  return {
    title,
    description,
    alternates: { canonical: `/hlk-jobs/${city.slug}` },
    openGraph: {
      title,
      description,
      url: `/hlk-jobs/${city.slug}`,
      type: "website",
      locale: "de_CH",
    },
  };
...
export default async function HLKCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findHLKCity(citySlug);
  if (!city) notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Startseite", item: SITE_URL },
      {
        "@type": "ListItem",
        position: 2,
        name: `Kältetechniker Jobs ${city.name}`,
        item: `${SITE_URL}/hlk-jobs/${city.slug}`,
      },
    ],
  };
  ...
  const itemListSchema = {
    ...
    "@type": "ItemList",
    name: `Kältetechniker Jobs ${city.name}`,
    description: `Offene Kältetechniker Stellen in ${city.name}`,
  ...
  const FAQS = [
    {
      question: `Wie viele Kältetechniker Stellen gibt es aktuell in ${city.name}?`,
      answer: `In ${city.name} und Umgebung sind regelmässig zwischen 20 und 120 offene Kältetechniker Stellen ausgeschrieben. Die Zahl schwankt mit Saison, Bautätigkeit und Auftragslage. Pendelbereite Bewerbende erweitern den Suchradius typischerweise auf ${city.commuterTowns.slice(0, 3).join(", ")} und finden so 2- bis 3-mal mehr passende Stellen.`,
    },
    {
      question: `Was verdient ein Kältetechniker in ${city.name}?`,
      answer: `Ein Kältetechniker verdient in ${city.name} typischerweise ${city.salaryBand} pro Jahr (12 × Monatslohn). Berufseinsteiger starten am unteren Ende, mit fünf Jahren Erfahrung und Spezialisierungen verschiebt sich der Lohn deutlich nach oben. Detaillierte Vergleichstabellen findest du auf der Lohnübersicht.`,
    },
    {
      question: `Welche Arbeitgeber suchen Kältetechniker in ${city.name}?`,
    ...
    {
      question: `Wie pendle ich am besten zu einem Kältetechniker Job in ${city.name}?`,
      ...
      <main className="bg-white">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            <nav className="text-sm text-slate-500 mb-3" aria-label="Breadcrumb">
              ...
              <span className="text-slate-700">Kältetechniker Jobs {city.name}</span>
            </nav>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight mb-4">
              Kältetechniker Jobs <span className="text-primary">{city.name}</span>
            </h1>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">
            Offene Kältetechniker Stellen in {city.name}
          </h2>
                ...
                <Card key={`${job.source}-${job.id}`} className="hover:border-primary/40 transition">
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-slate-600 mb-2 line-clamp-1">
                      {job.company || "Schweizer HLK-Betrieb"}
                    </p>
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Kältetechniker Berufe in {city.name}
            </h2>
        ...
        <section className="bg-slate-50 border-t">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-5">
              Häufig gestellte Fragen — Kältetechniker Jobs {city.name}
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            { HLK_CITIES.filter((c) => c.slug !== city.slug).map((c) => (
              <Link
                key={c.slug}
