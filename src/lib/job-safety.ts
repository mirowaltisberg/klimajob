...
export function classifyKlimaTitle(titleValue: unknown): JobSafetyResult {
  ...
  const hasTradeContext = matchesAny(title, safetyConfig.tradeContextPatterns);
  ...
  if (!match) return { disposition: "REJECT", reason: "no-klima-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-klima-title" : "broad-role-with-klima-context",
...
export function isValidKlimaJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getKlimaProfile(profile: JobSafetyProfile) {
