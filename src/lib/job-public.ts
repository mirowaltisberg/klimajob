...
import { classifyKlimaTitle, getKlimaProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifyKlimaTitle(input.title);
  if (
  ...
  const profile = getKlimaProfile(classification.profile);
