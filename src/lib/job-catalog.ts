...
import { classifyKlimaTitle, isValidKlimaJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifyKlimaTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
export async function searchJobListings(params: JobSearchParams): Promise<JobSearchResult> {
  ...
  const visibleRealCount = Math.min(total, normalized.offset + paged.length);
  const directHireOpportunities = buildDirectHireOpportunities({
    visibleRealCount,
    q: normalized.q,
    loc: normalized.loc,
