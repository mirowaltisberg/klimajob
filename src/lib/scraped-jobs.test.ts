...
const validId = `scraped-klima-${createHash("md5").update(jobUrl).digest("hex").slice(0, 12)}`;
...
test("mapped database rows pass only when their raw identity is valid", () => {
  ...
  assert.equal(mapVerifiedDbRow({ ...validRow, id: "scraped-klima-000000000000" }), null);
});
