"""Scrape a fresh chunk of genuine Swiss refrigeration and climate jobs into JSON.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_klima_title
...
def is_relevant_job(job: dict) -> bool:
    """Return True only for clearly refrigeration/climate-related Swiss roles."""
    ...
    return classify_klima_title(title)["disposition"] == "ACCEPT"
...
def _scrape_jobs_worker(result_queue, scrape_kwargs: dict) -> None:
    ...
    # JobSpy emits full request URLs at ERROR level even with verbose=0. Those
    # URLs contain the search keyword and location, so silence third-party
    # logging inside the isolated child and return only an exception type.
...
def scrape_swiss_jobs(
    ...
    """Scrape configured public listing sources for a query and location."""
...
def print_quality_summary(all_raw: list[dict]):
    """Print data quality stats for the publishable trade-specific rows."""
    normalized = []
    for i, raw in enumerate(all_raw):
        job = normalize_job(raw, i)
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Swiss refrigeration and climate jobs")
    ...
    if args.total_chunks > 1 and not args.query:
        ...
        selected = indexed_queries[args.chunk :: args.total_chunks]
