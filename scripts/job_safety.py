"""Shared deny-first Klima classifier backed by src/config/job-safety.json."""
...
def classify_klima_title(title_value: Any) -> dict[str, Any]:
    ...
    has_trade_context = any(
        ...
        for pattern in CONFIG["tradeContextPatterns"]
    )
    broad = None
    ...
    if not match:
        return {"disposition": "REJECT", "reason": "no-klima-title-signal"}
    ...
    return {
        "disposition": "ACCEPT",
        "reason": "specific-klima-title" if specific else "broad-role-with-klima-context",
