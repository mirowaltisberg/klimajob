"""Validate, merge and publish one complete climate-job snapshot.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_klima_title, validate_raw_job_identity
...
def is_recent_real_job(job: Any, cutoff: date, today: date) -> bool:
    ...
    if validate_raw_job_identity(job) is not None:
        return False
    if classify_klima_title(title)["disposition"] != "ACCEPT":
        return False
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish one validated climate-job snapshot")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Published {published} jobs and pruned {pruned} stale climate rows.")
        return 0
